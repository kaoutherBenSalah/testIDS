...
class IDSMonitor:
    ...
    def _handle_syn_packet(self, packet):
        if not packet.haslayer(IP) or not packet.haslayer(TCP):
            return

        self.stats["packets_seen"] += 1
        tcp_layer = packet[TCP]
        if tcp_layer.flags & 0x02 == 0:  # not a SYN
            return
        ...
        if (
            ...
            summary = (
                f"High SYN rate to {dst_ip}: {syn_count} SYNs/{self.syn_window_sec}s "
